...
///
/// `call`/`compose` throw `any Error`, and most of what escapes them is the
/// bound handler's own error passing through untouched. These two cases are the
/// only failures the *kernel itself* produces, and both mark a wiring-time
/// programming error rather than a runtime input: catch them to distinguish
/// "the machinery is miswired" from "the operation failed".
public enum KernelError: Error {
    /// No handler was wired for the given symbol id (a forgotten `register`).
    case unbound(String)
...
/// a `Symbol`'s phantom types with a concrete handler, and what the kernel's
...
///
/// (An app typically builds its own typed, human-readable index of callable
/// endpoints — a dispatch-key namespace — over these symbol ids. This is the
/// runtime cell those keys resolve to.)
public typealias ErasedHandler = @Sendable (Kernel, Any) async throws -> Verb<Any>
...
/// bindings into an immutable, `Sendable` `Kernel`.
///
...
public final class KernelBuilder {
    ...
    public var boundSymbolIDs: Set<String> { Set(handlers.keys) }
    ...
    public func register<P, O>(_ symbol: Symbol<P, O>, _ handler: @escaping @Sendable (P) async throws -> O) {
    ...
    /// other symbols (e.g. an orchestrating handler that dispatches down to
    /// leaf endpoints). Passing the kernel at call time, rather than wiring
    ...
    public func register<P, O>(_ symbol: Symbol<P, O>, _ handler: @escaping @Sendable (Kernel, P) async throws -> O) {
    ...
    public func register<P, O>(_ symbol: Symbol<P, O>, _ handler: @escaping @Sendable (P) async throws -> Verb<O>) {
    ...
    /// verb itself.
    public func register<P, O>(_ symbol: Symbol<P, O>, _ handler: @escaping @Sendable (Kernel, P) async throws -> Verb<O>) {
...
/// `kernel.call(Storage.Notes.fetch, id)`. Type safety is preserved
...
public final class Kernel: Sendable {
    ...
    /// Where a dispatched command's failure goes — wired by the composition
    /// root to the buffer.
    private let errorSink: @Sendable (any Error, String) async -> Void
    /// Where each symbol invocation is recorded (DEBUG only) — wired by the
    /// composition root to the buffer's `TraceState`. No-op by default, so
    /// release and tests pay
    ...
    public func call<P, O>(_ symbol: Symbol<P, O>, _ payload: P) async throws -> O {
    ...
    public func dispatch<P: Sendable, O>(_ symbol: Symbol<P, O>, _ payload: P) {
...
extension Kernel {
    ...
    public func call<O>(_ symbol: Symbol<Void, O>) async throws -> O {
